...
class default_1 {
    ...
    render(page, path, content) {
        ...
        //render
        const rootDiv = this.config.ssr ? global.ReactDOMServer.renderToString(global.React.createElement(FireJSX.app, { app: pagesCacheElement.app, content })) : "";
        //helmet
        if (this.config.ssr) {
    ...
    loadChunks(head, body, chunks, _require = true) {
        chunks.forEach(chunk => {
            const src = `${this.config.prefix}/${this.config.lib}/${chunk}`;
            if (chunk.endsWith(".js")) {
                if (this.config.ssr && _require)
                    require(path_1.join(this.config.outDir, this.config.lib, chunk));
